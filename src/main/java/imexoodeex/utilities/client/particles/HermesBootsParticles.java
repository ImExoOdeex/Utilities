...
public class HermesBootsParticles {
    ...
    private static void spawnParticles(LivingEntity entity, float random, World world, Vec3d left, Vec3d right) {
        ...
        if (!entity.isSubmergedInWater() && !isSneaking) {
            for (int i = 0; i < 10; i++) {
                world.addParticle(ParticleRegister.OVERSPEED, left.x, left.y + 0.1D, left.z, random, 0.0D, random);
                world.addParticle(ParticleRegister.SEC0ND_OVERSPEED, left.x, left.y + 0.1D, left.z, random, 0.0D, random);
                world.addParticle(ParticleRegister.OVERSPEED, right.x, right.y, right.z + 0.1D, random, 0.0D, random);
                world.addParticle(ParticleRegister.SEC0ND_OVERSPEED, right.x, right.y + 0.1D, right.z, random, 0.0D, random);
            }
        } else {
